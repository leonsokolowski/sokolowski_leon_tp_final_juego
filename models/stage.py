...
class Nivel:
    ...
    def spawnear_enemigos(self):
        if self.maxima_cantidad_enemigos > len(self.cordenadas_enemigos):
            ...
            for coordenada in range(self.maxima_cantidad_enemigos):
                self.minions.add(
                    Minion((self.cordenadas_enemigos[coordenada].get("coord_x"),self.cordenadas_enemigos[coordenada].get("coord_y")), self.limite_x, self.config_nivel))
    ...
    def run(self, delta_ms):
        ...
        for projectile in self.sprite_jugador.get_projectiles:
            ...
            pg.sprite.spritecollide(projectile, self.minions, True)
            ...
            if len(self.minions) == 0 and not self.__player_win:
